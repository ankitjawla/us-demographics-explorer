...
import { FIPS_TO_NAME } from "../../../lib/census.js";
 ...
 * tract — returning the app's geo_id for each so the UI can load
 ...
 * the caller should fall back to /api/geographies direct matches.
 */
...
export async function GET(req: Request) {
  try {
    ...
    if (cached) return Response.json({ suggestions: cached.slice(0, limit), cached: true });
    ...
    try {
      nominatim = await paced(() =>
        fetchJson(
          `${NOMINATIM}?q=${encodeURIComponent(q)}&format=jsonv2&addressdetails=1&countrycodes=us&limit=${limit}`
        )
      );
    } catch {
    ...
    for (const n of nominatim.slice(0, limit)) {
      try {
        const lat = n.lat;
        const lon = n.lon;
        if (!lat || !lon) continue;
