 ...
 *
 * Year-over-year change on the four headline indicators: current ACS 5-year
 * values (from `observations`) vs the 2019–2023 ACS 5-year baseline
...
export async function GET(req: Request) {
  try {
    ...
    let trends: Record<string, { current: number | null; previous: number | null; change: number | null; unit: "%" | "pp" }> | null = null;
    if (b) {
    ...
    return Response.json({ geo_id: geoId, baseline_release: "acs2023_5yr", trends });
  } catch (e) {
